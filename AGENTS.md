...
This is a personal project. By default, work directly on the current branch, which should usually be `main`.

Do **not** create a new branch, do **not** open a pull request, and do **not** turn the workflow into branch/PR-based collaboration unless the user explicitly asks for that.

When the user asks for version control actions, prefer direct `commit` + `push` to the intended branch instead of preparing a PR.
